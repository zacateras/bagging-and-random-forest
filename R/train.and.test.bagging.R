train_and_test_bagging <- function(df, df_name, ycolname, nrep=10, model_ntree=c(1, 200, 500, 700), model_mset=c(100, 1000, 10000), model_nfeatures=c(2, 3, 5, 7)) {
  library('ipred')
          ...
          for (node_size in model_node_size) {
            ...
            for (i in 1:nrep) {
              ...
              train_time <- train_time + system.time({
                classifier <- bagging( x=train[,xcolnames], 
                                            y=train[,ycolname], 
                                            nbagg=ntree,
                                            ns = mset
                )
