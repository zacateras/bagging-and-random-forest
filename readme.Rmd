...
train_and_test_set <- function(df, df_name, ycolname) {
  ...
  # more complex
  rpart_cms10_ccp1_fun <- function(formula, data) rpart(
    formula,
    data = data,
    control = rpart.control(minsplit = 10, cp = 1))
  train_and_test_ensemble(df, df_name,
                          rpart_cms10_ccp1_fun, 'rpart_cms10_ccp1', ycolname,
                          bag_nsets = c(10, 100), bag_nfeatures = c(3, 7))
  
  # less complex
  rpart_cms30_ccp0.001_cmd15_fun <- function(formula, data) rpart(
    formula,
    data = data,
    control = rpart.control(minsplit = 30, cp = 0.001, maxdepth = 15))
  train_and_test_ensemble(df, df_name,
                          rpart_cms30_ccp0.001_cmd15_fun, 'rpart_cms30_ccp0001_cmd15', ycolname,
                          bag_nsets = c(10, 100), bag_nfeatures = c(3, 7))
  
  # naiveBayes
  naiveBayes_fun <- function(formula, data) naiveBayes(
    formula,
    data = data)
  train_and_test_ensemble(df, df_name,
                          naiveBayes_fun, 'naivebayes', ycolname)
  
  # natural Laplace smoothing
  naiveBayes_l1_fun <- function(formula, data) naiveBayes(
    formula,
    data = data,
    laplace = 1.0)
  train_and_test_ensemble(df, df_name,
                          naiveBayes_l1_fun, 'naivebayes_l1', ycolname,
                          bag_nsets = c(10, 100), bag_nfeatures = c(3, 7))
  
  # Laplace smoothing with higher alpha
  naiveBayes_l10_fun <- function(formula, data) naiveBayes(
    formula,
    data = data,
    laplace = 10.0)
  train_and_test_ensemble(df, df_name,
                          naiveBayes_l10_fun, 'naivebayes_l10', ycolname,
                          bag_nsets = c(10, 100), bag_nfeatures = c(3, 7))
  
  # randomforest & bagging could be added here but were tested outside of this handbook environment
}
```
...
summary(covtype_df)
# train_and_test_set(covtype_df, 'covtype', 'Cover_Type')
```
